...
class NeuralSemiLagrangian(nn.Module):
    ...
    def forward(
        ...
        dynamic_features = dynamic_features.reshape(
            batch_size * self.dynamic_channels, 1, *dynamic_features.shape[-2:]
        )
        # Interpolate
...
class ForcingsIntegrator(nn.Module):
    ...
    def forward(
        self,
        hidden_features_dep: torch.tensor,
...
class Paradis(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Project departure and current states to latent space
        z_dep = self.to_latent(x_combined)
        z = self.to_latent(x)

        # Integrate forcings in latent space
        z = self.solve_along_trajectories(z_dep, z, self.dt)

        # Provide update with current state
        return x[:, : self.num_common_features] + self.from_latent(z)
