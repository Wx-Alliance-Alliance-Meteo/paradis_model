...
levels = ds.level.values if 'level' in ds.coords else [0]  # Handle datasets without 'level'
...
if 'prediction_timedelta' in ds.coords:
    ...
    forecast_hours = [int(delta / np.timedelta64(1, 'h')) for delta in prediction_deltas]
    deet = int(prediction_deltas[0] / np.timedelta64(1, 's'))  # Use the first timedelta value
else:
...
try:
    ig1234 = rmn.cxgaig('L', latitudes[0], longitudes[0], latitudes[1]-latitudes[0], longitudes[1]-longitudes[0])
    ...
    for t_idx, t_val in enumerate(time_steps):
        for fh_idx, forecast_hh in enumerate(forecast_hours):
            ...
            try:
                ...
                raise rmn.FSTDError(f"Failed to create/open the FST file: {fst_file_name}. Error: {str(e)}")
            ...
            for var_name in ds.data_vars:
                ...
                new_var_name = var_mapping.get("new_name", var_name)  # Default to the original name if no mapping exists
                ip1_value = var_mapping.get("ip1", 0)  # Default to 0 if no `ip1` value is defined
                conversion_formula = var_mapping.get("conversion")  # Retrieve conversion formula
                ...
                if 'time' in zarr_var.dims and 'level' in zarr_var.dims:
                    for level_idx, level_val in enumerate(levels):
                        ...
                        data_slice = np.asfortranarray(zarr_var.sel(
                            time=t_val,
                        ...
                        hhmmsshh = int(init_time.strftime("%H%M%S") + "00")  # Append two zeros
