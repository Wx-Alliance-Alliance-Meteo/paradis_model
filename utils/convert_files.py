...
def process_data(year, month, dpath, new_lat, new_lon, new_levels, output_base_dir):
    ...
    try:
        ...
        dset = xr.open_dataset(
            input_dataset_path, engine='zarr', chunks='auto')
        ...
        with dask.config.set(scheduler='threads'):
            regridded_dset.to_zarr(
                output_file_path, mode='w', consolidated=True)

        print(f"Rank {rank}: Successfully processed {input_dataset_path} -> {output_file_path} in {time.time() - t0:.2f} seconds")
...
def restack_data(year, files, output_base_dir):
    ...
    try:
        ...
        new_names = [val[0] + '_h' + str(val[1]) if str(val[1])
                     != 'nan' else val[0] for val in ds.stacked.values]
        ds = ds.drop_vars(['stacked', 'variable', 'level']
                          ).assign_coords(stacked=new_names)
        ...
        ds.attrs["note"] = "Variables have been renamed based on their original names and levels."
        ...
        print(f"Rank {rank}: Successfully processed {year} -> {output_file_path} in {time.time() - t0:.2f} seconds")
    except Exception as e:
...
def main():
    ...
    new_levels = [50,  100,  150,  200,  250,  300,
                  400,  500,  600,  700,  850,  925, 1000]

    # Specify the output base directory
    output_base_dir = '/home/cap003/hall5/gatmosphere_data'
    year_range = range(2016, 2024)
    file_lists = [os.path.join(output_base_dir, str(
        year), f'{month:02d}') for year in year_range for month in range(1, 13)]
    ...
    for i in range(0, len(file_lists), size):
        ...
        process_data(year, month, dpath, new_lat,
                     new_lon, new_levels, output_base_dir)
...
def convert_to_gatmosphere():
    ...
    files = [dpath + str(year) +
             '/*' for year in range(start_year, end_year+1)]
    year_files = [
        [glob_file for glob_file in glob.glob(file)] for file in files]
