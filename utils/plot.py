...
def visualize_predictions(lat, lon, true_data, output_data, output_dir="output"):
    ...
    feature_data_reshaped = (
        feature_data.detach().cpu().numpy().reshape(lat_size, lon_size)
    ...
    if output_data is not None:
        ...
        output_feature_data_reshaped = (
            output_feature_data.detach().cpu().numpy().reshape(lat_size, lon_size)
        )  # Detach, move to CPU and convert to numpy
